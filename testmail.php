...
while ($uemail = mysqli_fetch_assoc($res)) {
				...
				$result = mysqli_query($db,"SELECT col_useraccountsid as userid, col_dateofpurchase as date, col_totalprice as sales, col_transactionid as transactionid FROM tbl_transaction WHERE  col_dateofpurchase >= date('2018-10-01') AND col_dateofpurchase <= date('2018-10-30') AND col_useraccountsid = '".$uemail['col_useraccountsid']."' ORDER BY col_dateofpurchase ASC");
