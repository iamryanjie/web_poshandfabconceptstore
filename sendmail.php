...
$result = mysqli_query($db,"SELECT col_dateofpurchase as date, col_totalprice as sales, col_transactionid as transactionid FROM tbl_transaction WHERE  col_dateofpurchase >= date('2018-10-01') AND col_dateofpurchase <= date('2018-10-30') ORDER BY col_dateofpurchase ASC");
									
		while ($row = mysqli_fetch_assoc($result))
		 {
		 	$response = mysqli_query($db, "SELECT ps.col_productname, o.col_staticprice, o.col_quantitybought, o.col_subtotal, o.col_orderstatus FROM tbl_order as o JOIN tbl_product as ps ON ps.col_productid = o.col_productid where o.col_orderstatus='Sales' and ps.col_useraccountsid=20 and o.col_transactionid='".$row['transactionid']."'");
...
try{
mail('mojadofaye@gmail.com', 'try me', $message, $headers);
} catch (Exception $e) {
        echo $e;
}
?>
